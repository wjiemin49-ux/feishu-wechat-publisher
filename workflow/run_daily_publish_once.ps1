...
$Stdout = Join-Path $LogDir "daily_publish_$Stamp.stdout.log"
$Stderr = Join-Path $LogDir "daily_publish_$Stamp.stderr.log"
$CleanupLog = Join-Path $LogDir "daily_publish_$Stamp.cleanup.log"

$ExitCode = 0
Push-Location $RepoRoot
try {
    & $Python .\workflow\workflow.py scheduled-daily-run --platforms $Platforms --count $Count 1> $Stdout 2> $Stderr
